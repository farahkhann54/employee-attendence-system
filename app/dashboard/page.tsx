      ...
      getDocs(collection(db, "attendance")).then((attendanceSnap) => {
        getDocs(collection(db, "users")).then((userSnap) => {
          const active = userSnap.docs
            .filter((entry) =>
              attendanceSnap.docs.some(
                (item) =>
                  item.data().userId === entry.id &&
    ...
    () => [
      {
        ...
        tone: "from-cyan-500 via-blue-500 to-blue-600",
        color: "text-white",
        subtext: "Current",
      },
      {
        ...
        value: shiftDuration,
        icon: Clock3,
        tone: "from-purple-500 via-purple-600 to-purple-700",
        color: "text-white",
        subtext: "Today",
      },
      {
        label: "Live Staff",
        value: String(liveStaff.length).padStart(2, "0"),
        icon: Users,
        tone: "from-emerald-400 via-emerald-500 to-teal-600",
        color: "text-white",
        subtext: "Online",
      },
      {
        ...
        icon: ShieldCheck,
        tone: "from-orange-400 via-orange-500 to-red-500",
        color: "text-white",
        subtext: "Attendance",
      },
    ],
              ...
              >
                ...
                <div className="relative z-10 flex flex-col justify-between">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-[0.65rem] font-bold uppercase tracking-wider text-white/75">
                        {item.label}
                      </p>
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white via-slate-50/50 to-slate-50 p-6 shadow-lg sm:p-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              ...
              <h3
                className={`mt-2 text-3xl font-black capitalize ${currentStatus === "active" ? "text-cyan-600" : currentStatus === "break" ? "text-purple-600" : "text-slate-600"}`}
              >
                {currentStatus}
              </h3>
              ...
              <ResponsiveContainer width="100%" height={350}>
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={true}
                    // FIX: Added (percent ?? 0) to handle potential undefined values safely
                ...
                liveStaff.map((member, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-xl font-bold ${member.uid === user?.uid ? "bg-gradient-to-br from-cyan-500 to-blue-600 text-white" : "bg-gradient-to-br from-slate-200 to-slate-300 text-slate-700"}`}
                      >
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-950">
                          ...
                          {member.uid === user?.uid && (
                            <span className="ml-2 rounded-full bg-cyan-100 px-2 py-0.5 text-[0.6rem] font-bold text-cyan-700">
                              (You)
                            </span>
                          )}
                        </p>
            ...
            <div className="relative z-10 flex flex-col justify-between gap-6">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-white/80">
                  Manage your attendance, breaks, and checkout with a single
                  click.
                </p>
              </div>
